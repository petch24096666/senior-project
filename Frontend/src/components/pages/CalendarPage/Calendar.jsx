...
const CalendarDashboard = () => {
  ...
  const [microsoftToken, setMicrosoftToken] = useState(
    localStorage.getItem("microsoftToken") || null
  );
    ...
    if (provider === "google") {
      ...
      if (response.ok) {
        ...
        if (data.items && Array.isArray(data.items)) {
          ...
          console.error(
            "❌ Google events data.items is not an array or is empty"
          );
        }
      } else {
      ...
      const response = await fetch(
        "http://localhost:8081/api/microsoft-events",
        {
      ...
      if (response.ok) {
        ...
        if (data.value && Array.isArray(data.value)) {
          ...
          console.error(
            "❌ Microsoft events data.value is not an array or is empty"
          );
        }
      } else {
  ...
  const handleAddEvent = async () => {
    ...
    const start = DateTime.fromISO(newEvent.start)
  ...
  const handleEventDrop = async (info) => {
    ...
    if (provider === "google") {
      ...
      try {
        const response = await fetch(
          `http://localhost:8081/api/update-google-event/${eventId}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!response.ok)
          throw new Error(`Google update failed: ${response.status}`);
        const data = await response.json();
      ...
      try {
        ...
        if (!response.ok)
          throw new Error(`Microsoft update failed: ${response.status}`);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      if (provider === "google") {
        ...
        const response = await fetch(
          `http://localhost:8081/api/delete-google-event/${eventId}`,
          {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${googleToken}`,
  ...
  const getGoogleAccessToken = async () => {
    try {
      const response = await fetch(
        "http://localhost:8081/api/refresh-google-token"
      );
      const data = await response.json();
    ...
    <div style={styles.container}>
                ...
                .map((event, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span style={{ ...styles.taskTime, color: timeColor }}>
                        {timeLabel}
                      </span>
                    </li>
                  );
      ...
      <div style={styles.mainContent}>
        <ToggleButtonGroup
          value={view}
          exclusive
          onChange={(event, newView) => setView(newView || "timeGridDay")}
        >
