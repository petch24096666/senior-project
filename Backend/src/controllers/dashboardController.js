...
export const getDashboardProjects = async (req, res) => {
    ...
    try {
      // ดึงโปรเจกต์ที่ผู้ใช้เป็น creator หรือเป็นสมาชิก
      const sql = `
        SELECT DISTINCT p.*
        FROM projects p
      ...
      const projects = rows.map(project => ({
        ...
        team: [] // จะนำข้อมูล team มาเติมต่อไป
      }));
  ...
  export const getActiveTasksCount = async (req, res) => {
    ...
    try {
      // Modified query to count active and completed tasks directly
      const sql = `
        SELECT 
          COUNT(*) AS totalTasks,
  ...
  export const getAssignedTasks = async (req, res) => {
    ...
    try {
      ...
      // แสดง query ใน console สำหรับ debugging
