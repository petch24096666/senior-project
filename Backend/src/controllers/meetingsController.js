...
import { v4 as uuidv4 } from 'uuid'; // ✅ เพิ่มบรรทัดนี้
...
export const handleZoomCallback = async (req, res) => {
  ...
  try {
    ...
    await db.query(`
      UPDATE users
      SET token = ?, refresh_token = ?, provider = 'zoom'
      WHERE email = ?
  ...
  try {
    ...
    if (cleanPlatform === "zoom") {
      const [[user]] = await db.query(
        "SELECT token, refresh_token FROM users WHERE user_id = ?",
        [user_id]
      );

      if (!user?.token) {
        return res.status(401).json({ error: "Zoom not authenticated." });
      }
      ...
      try {
        ...
        if (error.response?.status === 401) {
          ...
          await db.query(
            "UPDATE users SET token = ?, refresh_token = ? WHERE user_id = ?",
            [accessToken, refresh.data.refresh_token, user_id]
          );
      ...
      const domainMap = {
        webex: "webex.com/meeting/",
        "microsoft teams": "teams.microsoft.com/l/meetup-join/",
        "google meet": "meet.google.com/",
      };

      const domain = domainMap[cleanPlatform];
      if (!domain) {
        return res.status(400).json({ error: "Unknown platform" });
      }

      join_url = `https://${domain}${meetingId}`;
    }
...
export const getAllMeetings = async (req, res) => {
  ...
  try {
    const [meetings] = await db.query(
      `SELECT * FROM meetings WHERE user_id = ? ORDER BY date, time`,
      [userId]
    );

    res.json(meetings);
  } catch (err) {
    console.log("📥 Fetching meetings for user:", req.params.userId);
...
export const updateMeetingById = async (req, res) => {
  ...
  try {
    await db.query(
      `UPDATE meetings SET title = ?, date = ?, time = ?, duration = ? WHERE id = ?`,
      [title, date, time, duration, id]
    );
